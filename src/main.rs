...
impl ZellijPlugin for State {
    fn load(&mut self, _configuration: BTreeMap<String, String>) {
        self.loading = true;
        request_permission(&[PermissionType::ChangeApplicationState, PermissionType::OpenFiles, PermissionType::OpenTerminalsOrPlugins]);
        subscribe(&[
            EventType::Key,
            EventType::Mouse,
        ...
        post_message_to(PluginMessage {
            worker_name: Some("file_name_search".to_owned()),
            name: serde_json::to_string(&MessageToSearch::ScanFolder).unwrap(),
            payload: "".to_owned(),
        });
        post_message_to(PluginMessage {
            worker_name: Some("file_contents_search".to_owned()),
            name: serde_json::to_string(&MessageToSearch::ScanFolder).unwrap(),
            payload: "".to_owned(),
        });
